...
#개요
자연어 생성(NLG) 모델에서 사용되는 주요 디코딩 알고리즘을 학습하고 이해한 내용을 정리한 문서로
각 기법의 개념, 장단점 , 작동방식 등을 공부한 내용입니다
...
- 모델은 다음 확률을 예측한다.
<img width="177" height="63" alt="image" src="https://github.com/user-attachments/assets/2e0e3aea-2622-40be-b5f6-c77882d533ec" />
...
4.다음 스텝에서는 두 문장 각각을 모든 단어로 확장해서
  총 8개의 후보를 만들고,그중 상위 2개만 남긴다.

---

### 특징

장점: 1.그리디 서치 디코딩 보다 더 좋은 품질의 문장을 생성 가능

      2.초기 선택 실수의 영향이 적어 전역 최적에 더 가까움
           
단점: 1.빔 크기가 클수록 생성 과정이 느려짐
...
### 반복 문제 해결
단순한 그리디 서치보다 빔 서치에서 더 높은 로그 확률을 얻었습니다.(높을수록 좋은거) 하지만 빔 서치도 텍스트가 반복되는 문제가 있습니다. 
이 문제를 해결하기 위해 no_repeat_ngram-size 매개변수로 n-그램 패널티를 부과하는 방법이 있음
...
### 개요
샘플링 기반 디코딩은 매 스텝에서 모델이 출력한 확률 분포에서 확률적으로 토큰을 무작위 선택하는 방식이다.
Gready Search처럼 1등 단어만 고르지 않고, Beam Search처럼 상위 b개만 고르지 않는다.
즉, 확률이 높을수록 뽑힐 가능성이 높지만, 낮은 확률의 단어도 일정 확률로 선택될 수 있는 방식

그래서 동일한 입력이라도 매번 다른 문장이 나올 수 있다.
...
<img width="203" height="50" alt="image" src="https://github.com/user-attachments/assets/6730e753-b5a7-4e03-9743-9d9bc4bea9e5" />

-이 확률을 softmax에서 얻은 분포 pi라고 하면, 샘플링은 다음처럼 확률 분포에서 무작위로 토큰을 고른다.

<img width="359" height="54" alt="image" src="https://github.com/user-attachments/assets/a507d3bc-266d-4766-8c5b-d503d445e27f" />

Categorical 분포 = 다항분포에서 하나 뽑은것

V = 전체 어휘수

확률이 큰 단어는 더 자주 선택되지만 작은 단어도 선택 될수 있음


### 특징
- **장점**: 매번 다른 문장을 만들 수 있고, 창의적인 문장과 다양성이 생김 == 자연스러운 대화/스토리
            또한 Gready 보다 반복문제가 적음
       
- **단점**: 확률이 너무 퍼져 있으면 엉뚱한 문장도 생성될 수 있고 품질이 불안정 할수가 있음
...
### Temperature 적용된 샘플링 수식

-- Temperature T를 사용하면 확률 분포를 더 날카롭게 또는 더 평탄하게 만들수 있다.

<img width="843" height="85" alt="image" src="https://github.com/user-attachments/assets/e86a791d-33a4-4848-a405-ab167b8b76de" />

-- T < 1 -> 큰 확률이 더 커지고 작은 확률이 더 작아짐 == 보수적,안정적 문장

-- T > 1 -> 확률 분포가 평탄해짐  == 창의적 ,랜덤성 증가
...
디코딩 알고리즘을 공부하면서, 모델이 텍스트를 생성한다는 것이 단순히 “다음 단어를 예측하는 것” 이상이라는 점을 깊이 이해하게 되었다. 특히 Greedy Search, Beam Search, Top-K, Top-P와 같은 방법들이 문장 품질과 다양성 사이에서 어떤 선택을 하는지 비교해보며 “디코딩 전략에 따라 같은 모델도 전혀 다른 성격의 문장을 생성한다”는 점이 흥미로웠다. 또한 샘플링 기반 방식의 확률적 특성, 반복 제거 기법(no-repeat-ngram)의 필요성 등을 확인하면서 실제 텍스트 생성에서 품질을 높이는 과정이 이론보다 훨씬 복잡하다는 것을 느꼈다. 단순히 모델 파라미터나 구조가 아니라 디코딩 전략의 선택이 결과물의 스타일을 결정한다는 사실을 체감할 수 있었고, 이를 통해 LLM의 동작 원리를 좀 더 깊게 이해하게 되었다. 앞으로 다른 디코딩 기법들과 실제 태스크에서의 성능 차이도 실험해보고 싶다는 동기부여가 되었다.
